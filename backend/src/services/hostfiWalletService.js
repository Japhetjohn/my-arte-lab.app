...
class HostFiWalletService {
  ...
  async syncWalletBalances(userId) {
    try {
      ...
      // Update aggregate balance: Use HostFi as the base, but allow for a ledger "cushion"
      // if it's specifically for non-HostFi manual restorations.
      // However, to prevent inflation, we should generally trust the aggregate of assets.
