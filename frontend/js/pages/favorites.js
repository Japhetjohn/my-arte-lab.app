...
export async function renderFavoritesPage() {
    ...
    mainContent.innerHTML = `
        <div class="section">
            <div class="container">
                <h1 class="page-title">Saved Creators</h1>
                <div class="text-center" style="padding: 60px 20px;">
                    ...
                    <p class="text-secondary">Loading your favorites...</p>
                </div>
            </div>
        </div>
    `;
                    ...
                    <div class="section">
                        <div class="container">
                            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 32px;">
                                <h1 class="page-title">Saved Creators</h1>
                                <span class="badge">${favorites.length} ${favorites.length === 1 ? 'Creator' : 'Creators'}</span>
                            </div>
                    ...
                    <div class="section">
                        <div class="container">
                            ...
                            <div class="empty-state">
                                ...
                                <button class="btn-primary" onclick="window.navigateTo('discover')" style="margin-top: 16px;">Discover Creators</button>
                            </div>
                        </div>
                    </div>
                `;
...
function renderFavoriteCard(creator) {
    ...
    return `
        <div class="creator-card" data-creator-id="${creator._id}" style="border: 1px solid var(--border); border-radius: 12px; padding: 20px; background: var(--surface); cursor: pointer; transition: all 0.2s;">
            <div style="display: flex; align-items: center; gap: 16px; margin-bottom: 16px;">
                <img src="${avatar}" alt="${creator.name}" style="width: 60px; height: 60px; border-radius: 50%; object-fit: cover;">
                <div style="flex: 1;">
                    <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
                        <div style="font-weight: 600; font-size: 16px;">${creator.name}</div>
                ...
                <button class="btn-ghost unfavorite-btn" data-creator-id="${creator._id}" style="padding: 8px; min-width: auto;">
            ...
            <!-- Performance Metrics -->
            ${creator.metrics ? `
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 16px; padding: 12px; background: var(--background); border-radius: 8px;">
            ...
            ${creator.badges && creator.badges.length > 0 ? `
                <div style="display: flex; gap: 6px; flex-wrap: wrap; margin-bottom: 16px;">
                    ${creator.badges.slice(0, 3).map(badge => {
                        ...
                        return `<span style="font-size: 12px; padding: 3px 8px; background: var(--background); border-radius: 4px;">${info.icon} ${info.label}</span>`;
                    }).join('')}
                </div>
            ` : ''}
            ...
            <div style="display: flex; gap: 8px;">
                ...
                <button class="btn-secondary book-btn" data-creator-id="${creator._id}" style="flex: 1; font-size: 14px;">Book Now</button>
            </div>
        </div>
    `;
}
